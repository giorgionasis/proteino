...
Last updated: 2026-05-13 (session 22 — admin-controlled page layouts + related sections + landscape list)

---

## 0. WHERE WE LEFT OFF (read first when resuming)

**Current state — session 22 (current) finished:**
...
### Phase A.6 — Open design calls (post-session 22, still pending)

These weren't blocked from shipping but were intentionally deferred during session 22:

- **postMessage scroll-to-section in the layout preview iframe.** Today clicking a section in the admin stack doesn't scroll the iframe to that section. Nice-to-have UX polish, ~30 min to wire. Pattern: admin sends `{type:'scroll',sectionId}` via `iframe.contentWindow.postMessage`; preview page listens on `window` + scrolls + highlights.
- **`item_detail` context in `page_sections` vs. separate `related_sections_config` table.** Session 22 deliberately built related sections as a separate config table rather than fitting them into `page_sections` with `context='item_detail'` — simpler admin UX, matches the "global rule" mental model. The forward-compat alternative is documented in PROGRESS.md but not implemented. Revisit only if admin needs reorder / drag / variant per item.
- **Static-carousel item-source picker.** `WidgetSpec.configSchema` has an `item-source` field type that's currently a placeholder. The static_carousel widget on home/category supports four source presets (`top_rated` / `newest` / `most_bookmarked` / `most_reviewed`) via the select field, but no rich UI for picking a specific item set. Probably becomes necessary if admin wants to build manual "Editor's picks" carousels without going through Collections.
- **CLAUDE.md §37 Layout system + §38 Related Sections** — documented in this session. Should re-check during the next session that no edge case is unexplained (e.g. how `static_carousel` on home picks the right portrait/landscape variant based on `config.category`).
